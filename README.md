...
| V5 | **Depth** | The camera moves through the collections: every collection sits in the same place at its own depth, and one value places the camera. Down the rail it pushes deeper: the page you leave passes you (toward 1.045, a whisper of blur, a soft shadow), the next surfaces from behind (0.92 → 1), sharp from its first frame. Up the rail is the mirror. **Real perspective:** every card scales about one vanishing point out by the folders, so deeper pages are drawn toward the folder they live in. **Depth wave:** the heading and the cards nearest the rail lead, the far column and lower row follow. **Heading rides with its page**, and the two names take turns rather than overlapping. **Photos settle into their frames** a beat after the card. **Scroll moves the camera:** overscrolling leans you toward the next collection, which surfaces behind the page; let go and it eases back, cross the threshold and the switch continues from where you are. A flick shortens the trip by up to a quarter; a reversal mid-flight heads straight back on a quick-start curve with no opacity pops; a two-step jump flies through the middle collection as a blurred ghost. V3's curve, cubic-bezier(0.3, 0.05, 0.05, 1) over 0.9s. | Moving the camera through the collections |
...
- **Hovering a folder opens its card stack** slightly, and **selecting one makes its cards hop** — the front card lifts and tips, the back two follow 40ms later, and they land with a small dip (click or keyboard; scroll switches stay still). Pressing a tab squeezes its cards to 0.96.
