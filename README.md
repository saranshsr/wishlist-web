...
| V5 | **Depth** | Real depth tied to where you're going: every collection sits in the same place at its own depth and one value moves the camera. Down the rail it goes deeper — the page you leave drifts toward you (1.04) and fades with a whisper of blur, the next emerges from behind (0.94 → 1), sharp from its first frame. Up the rail is the mirror. One strong ease-out, cubic-bezier(0.23, 1, 0.32, 1) over 0.46s, that stops clean; no edge band — pages scale from their top edge so they never touch the title. | Moving the camera through the collections |
