...
| V5 | **Depth** | Real depth tied to where you're going: every collection sits in the same place at its own depth and one value moves the camera. Down the rail it goes deeper — the page you leave drifts toward you (1.045) and fades with a whisper of blur, casting a shadow while it is nearer than the panel; the page ahead is hazed like distance and its rows sit at slightly different depths, the next emerges from behind (0.92 → 1), sharp from its first frame. Up the rail is the mirror. V3's timing — cubic-bezier(0.3, 0.05, 0.05, 1) over 0.9s, a soft start and long glide; no edge band — pages scale from their top edge so they never touch the title. | Moving the camera through the collections |
